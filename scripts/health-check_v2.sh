...
# Add missing services with descriptions
...
echo -e "${CYAN}======cc    =====================================${RESET}"

# Capture the output of docker ps into an array
IFS=$'\n' read -d '' -r -a containers <<<"$(docker ps --format "{{.Names}} {{.Status}} {{.Ports}}")"
echo -e "${CYAN}==== Health Docker =============${RESET}"
echo -e "${CYAN}=== IFS= $IFS ====${RESET}"
echo -e "${CYAN}============================================${RESET}"
